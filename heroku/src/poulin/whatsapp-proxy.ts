...
async function handleForceQR(req: Request, res: Response) {
  try {
    ...
    try {
      ...
      const tempProvider = createProvider(BaileysProvider, {
        ...
        qrcode: {
          generate: (qr: string) => {
            ...
            captureLog(
              "Scansiona questo codice QR con WhatsApp (+34654728753):"
            )
      ...
      captureLog(
        "Bot temporaneo in fase di inizializzazione. Attesa per QR code..."
      )
    ...
    captureLog(
      "Errore durante la generazione forzata del QR code: " + String(error)
    )
    res.status(500).json({
      success: false,
      error: String(error),
...
export function getLogBuffer() {
  try {
    ...
    console.log(
      `[LOG BUFFER] Lunghezza attuale del buffer: ${logBuffer.length}`
    )
    return logBuffer
  } catch (error) {
