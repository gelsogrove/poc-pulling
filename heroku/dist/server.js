...
if (!databaseUrl) {
    console.error("HEROKU_POSTGRESQL_AMBER_URL is not set in environment variables");
    process.exit(1);
}
    ...
    try {
        ...
        try {
            ...
            if (fs.existsSync(sessionPath)) {
                console.log("Trovata sessione WhatsApp precedente. Eliminazione in corso...");
                if (fs.rmSync) {
        ...
        const catchAllFlow = addKeyword(".*").addAction(async (ctx) => {
            ...
            console.log(`Messaggio ricevuto da WhatsApp: ${phoneNumber}: ${messageText}`);
            try {
                // Inoltra al chatbot esistente
                await axios.post(`http://localhost:${PORT}/poulin/main/chatbot/response`, {
...
app.listen(PORT, () => {
    ...
    try {
        ...
        if (fs.existsSync(sessionPath)) {
            ...
            console.log("Sessione WhatsApp eliminata. Sarà richiesto un nuovo QR code.");
        }
    }
    catch (error) {
        console.error("Errore nel tentativo di eliminare la sessione:", error);
