...
const createModel = async (req, res) => {
    ...
    try {
        const result = await pool.query("INSERT INTO Models (model, note) VALUES ($1, $2) RETURNING *", [model, note || null]);
...
const getModels = async (req, res) => {
    ...
    try {
        const result = await pool.query("SELECT * FROM models ORDER BY idmodel DESC");
...
const updateModel = async (req, res) => {
    ...
    try {
        const result = await pool.query("UPDATE Models SET model = $1, note = $2 WHERE idmodel = $3 RETURNING *", [model, note || null, id]);
