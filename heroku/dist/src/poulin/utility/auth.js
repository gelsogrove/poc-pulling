...
const registerHandler = async (req, res) => {
    ...
    const result = await pool.query("INSERT INTO users (username, name, surname, password, expire_date, role, otp_secret) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING userId, username, name, role", [username, name, surname, hashedPassword, null, "User", secret.base32]);
...
const setExpire = async (req, res) => {
    ...
    const { rows } = await pool.query("SELECT expire_date FROM users WHERE userid = $1 AND isactive = true", [userId]);
...
const isExpired = async (req, res) => {
    ...
    try {
        ...
        const isExpired = new Date(expire_date) < new Date() && rows[0].isactive === false;
...
const logoutHandler = async (req, res) => {
    ...
    try {
        ...
        await pool.query("UPDATE users SET token = NULL, expire_date = NULL, isactive = false WHERE userid = $1", [userId]);
...
const getClientHandler = async (req, res) => {
    try {
        ...
        const { rows } = await pool.query("SELECT userid,name,role,username,surname,expire_date,token,isactive FROM users WHERE userid = $1 ", [userId]);
        if (rows.length === 0) {
            res.status(404).json({ message: "Utente non trovato" });
            return;
        }
