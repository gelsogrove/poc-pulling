 ...
 * Specializzato per la gestione logistica e magazzino
 */
...
const logisticsRouter = Router(); // Rinominato per chiarezza
// Verifica configurazione API
if (!process.env.OPENROUTER_API_KEY) {
    throw new Error("OPENROUTER_API_KEY is not set in the environment variables.");
}
/**
 * Gestisce le richieste al chatbot per la logistica
 *
 * Funzionalità specifiche:
 * - Tracciamento spedizioni e consegne
 * - Gestione magazzino e inventario
 * - Ottimizzazione percorsi di consegna
 * - Analisi performance logistiche
 *
...
const handleResponse = async (req, res) => {
    ...
    `;
        ...
        // Preparazione payload con contesto logistica
        ...
        try {
            ...
            // Esecuzione query per dati logistica
            ...
            // Risposta standard con dati logistica
            res.status(200).json({
                triggerAction,
...
logisticsRouter.post("/response", handleResponse);
