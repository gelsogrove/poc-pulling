...
import { validateRequest, validateUser } from "../share/validateUser.js"; // Presuppone che queste funzioni esistano
import { sendUsageData } from "../utility/chatbots_utility.js";
const unlikeRouter = Router();
/**
 * Endpoint per creare un nuovo record di unlike.
 */
const createUnlikeHandler = async (req, res) => {
    const { userId, token } = await validateRequest(req, res);
    if (!userId)
        return;
    const { conversationId, msgId, dataTime, conversationHistory, idPrompt, model, temperature, } = req.body;
    if (!conversationId ||
        !msgId ||
        !dataTime ||
        !conversationHistory ||
  ...
  `;
    ...
    `;
            ...
            res.status(200).json({
                message: "Record already existed and has been deleted. No new record inserted.",
            });
            return;
        }
...
const getUnlikeHandler = async (req, res) => {
    ...
    `;
        ...
        console.error("Error fetching records:", error instanceof Error ? error.message : error);
        res.status(500).json({ error: "Internal server error" });
    }
};
/**
 ...
 */
const deleteUnlikeHandler = async (req, res) => {
    const { userId, token } = await validateRequest(req, res);
    if (!userId)
        return;
    const { id } = req.params;
  ...
  `;
        ...
        console.error("Error deleting record:", error instanceof Error ? error.message : error);
        res.status(500).json({ error: "Internal server error" });
    }
};
/**
 ...
 */
const checkUnlikeExistsHandler = async (req, res) => {
    const { userId, token } = await validateRequest(req, res);
    if (!userId)
        return;
    const { idPrompt } = req.query;
    ...
    `;
        ...
        console.error("Error checking unlike existence:", error instanceof Error ? error.message : error);
        res.status(500).json({ error: "Internal server error" });
    }
};
